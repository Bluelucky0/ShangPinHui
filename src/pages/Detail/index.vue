...
export default {
  ...
  computed: {
    ...
    //父给子传值，直接在标签里面传值，控制台会报红，原因是当skuInfo.skuImageList数据还没回来的时候数组值是undefined，而子接收到一个空数组undefined，取值的时候会报错
    //控制台的报错不会影响程序的运行，但是控制台会报红
    //所以我们需要对这个数组进行加工，至少保证他是一个空数组，
    skuImageList() {
  ...
  methods: {
    //排他思想：干掉所有人，留下我自己
    //首先要拿到点击的那个元素，还要拿到点击元素的父元素进行遍历，把所有的高亮样式清掉，然后点击的那个添加高亮
    changeArr(spuSaleAttrValue, arr) {
      arr.forEach((item) => {
        item.isChecked = 0;
      });
    ...
    //输入框改变时绑定事件
    skuNumChange(event) {
      //输入框的值*1，因为输入框的值是字符串，而字符串中只要含有不是数字的值然后*1之后返回的都是NaN
      let value = event.target.value * 1;
      // console.log(value);
      //非法输入的判断---输入非number或输入为负值
    ...
    //添加购物车
    async addShopCart() {
      //携带参数skuid（通过路由获取）和skuNum（通过上面输入框绑定的value值获取） 通过仓库发请求
      try {
        //派发dispatch实质就是调用addShopCart函数，而这个函数是在仓库调用，我们需要拿到调用的结果进而进行相应的操作
        //而这个函数调用返回的是一个Promise对象，所以我们可以用await 拿到这个函数调用成功的返回值，然后用try...catch去捕获错误
        await this.$store.dispatch("addShopCart", {
          skuid: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //请求成功进行路由跳转
        //因为skuInfo是一个对象，如果用query参数的话，地址栏就会出现一大长串，所以我们使用H5新增的会话存储（sessionStorage，页面关闭之后就会消失）来保存
        //简单的参数使用query参数传过去，复杂的参数我们借用会话存储，在发请求之前对复杂的数据先存储到本地
        //需要注意的一点就是无论是本地存储还是会话存储都不能存储对象，只能存储字符串！！！所以要进行转化
